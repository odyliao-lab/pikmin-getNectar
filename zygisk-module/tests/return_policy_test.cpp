...
int main() {
    ...
    assert(return_task_ready(1,0,true,0,now)); // live carried fruit omits requirement
    ...
    assert(!return_task_ready(6,0,true,0,now));
    assert(!return_task_ready(9,0,true,0,now));
    assert(!return_task_ready(1,-1,true,0,now)); // getter missing, not protobuf default
    assert(!return_task_ready(1,0,false,0,now));
    ...
    std::puts("return policy: 24 checks passed");
}
