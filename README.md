...
遊戲更新通常會改變 IL2CPP RVA；版本不是 152.0 時請不要安裝或啟用。模組也會驗證 `libil2cpp.so` 檔案大小，不相符時不安裝 hooks。使用 root、定位模擬或遊戲自動化可能違反服務條款，帳號與裝置風險由使用者自行承擔。

## 安裝

1. 從 [`dist/pikmin-nectar-rpc-v152.zip`](dist/pikmin-nectar-rpc-v152.zip) 安裝 Magisk 模組並重新開機。
