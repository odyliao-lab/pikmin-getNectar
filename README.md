...
最新候選1.4.21/code55搭配CC0.6.15/code32：一般附近派遣必須同時符合遊戲處理後位置200m、定位同步穩定、跨tick同隊重驗及原生往返≤120秒，防止手動長距離GPS分段追趕時提前派遣。保留3筆並行；批次／花田既有流程不改。1.4.20實機已驗證定位欄位，但發現遊戲時間屬於開機時鐘，誤與Unix時間比較而拒派；1.4.21修正時鐘基準，7組原生policy測試通過、Magisk更新已排入。**仍待再次重開機與真正定位／派遣驗收；目前附近派遣off**。[安全規則、備份及待驗證項目](NEARBY_SAFETY_2026-09-06.md)。
