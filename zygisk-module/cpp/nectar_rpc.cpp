...
// ExpeditionItemData.get_Pikmins exposes the game's persisted selection as
// IEnumerable<string>. This only enumerates it for diagnostics; it does not
// hook the UI or mutate the selection.
std::string expedition_pikmin_ids(void *data, int *selected_count) {
    if (selected_count) *selected_count = 0;
    if (!data || !get_expedition_pikmins || !object_get_virtual_method || !class_get_method_from_name) return {};
    ...
    for (int index = 0; index < count; ++index) {
        ...
        if (batch_selection_pending) {
            if (id_text == batch_selection_settling_id &&
                ...
                if (settled_gate) {
                    int persisted_count{};
                    const std::string persisted_ids = expedition_pikmin_ids(data, &persisted_count);
...
void install_return_diagnostic_hook() {
    ...
    LOGI("[DISPATCH-OBSERVE] persisted selection getter=%p", pikmins_method);
