...
// CALL_PREPARE_ACTION=2. Only extend the legacy due-time path for explicit
// carried resources that the server says need no preparation. Never infer
...
inline bool collectable_carry_resource(int task_case, int requirement,
    ...
    return task_case == 1 && requirement == 1 && has_resource &&
